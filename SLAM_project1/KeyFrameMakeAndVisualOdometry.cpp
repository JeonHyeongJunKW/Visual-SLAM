...
int main(int argc, char** argv)
{
  ...
  while(true)
  {
    ...
    if(nodehandler.estimated_pose.size()-last_size>0)
    {
      ...
      int estimated_col = 10+(last_estimated_pose.at<float>(0,3)-min_point_x)*1;//추정된 x좌표를 그림좌표로 바꾼것
      int estimated_row = 10+(last_estimated_pose.at<float>(2,3)-min_point_z)*1;//추정된 z좌표를 그림좌표로 바꾼것
